...
fn pre_process(mut input: Vec<u8>) -> Vec<u8> {
    return input;
}


fn get_chunks(padded_input: Vec<u8>) -> Vec<[u8; 512]> {
    return Vec::new();
}


pub fn sha_256(input: String) -> String {
    let padded = pre_process(input.bytes().collect::<Vec<u8>>());
    let chunks = get_chunks(padded);
